		...
		if ( !bRes )
		{
			...
			else if ( errCode == ERROR_PIPE_CONNECTED )
			{
				if ( g_pPipeServer->m_pCallBack != NULL && !(g_pPipeServer->m_bUserConnected) )
				{
					...
					{
						g_pPipeServer->m_pCallBack(1,(unsigned int)g_pPipeServer);
					}
				}
			}
...
BOOL CPipeServer::StartServer(const  TCHAR *pPipeName, P_NOTIFY_CALLBACK callbackFun )
{
	...
	SvrHandle = CreateNamedPipe( pipenamestr, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED, PIPE_NOWAIT | PIPE_TYPE_BYTE , 1, PIPE_BUFFER_SIZE, PIPE_BUFFER_SIZE, 1000, &sa );
